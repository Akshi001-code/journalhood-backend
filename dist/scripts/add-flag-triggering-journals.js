...
async function addFlagTriggeringJournals() {
    ...
    const issueTypes = Object.keys(mentalHealthTemplates);
    let studentIndex = 0;
    let totalEntries = 0;
    for (let i = 0; i < issueTypes.length && studentIndex < students.length; i++) {
        ...
        for (const student of studentsForThisIssue) {
            for (const template of templates) {
                ...
                const timestamp = firebase_config_1.default.firestore.Timestamp.fromDate(entryDate);
                const entry = {
                ...
                const encryptedContent = (0, encrypt_decrypt_1.encryptWithUid)(JSON.stringify(contentDelta), student.uid);
                const encryptedEntry = {
                    ...entry,
                    content: encryptedContent
                };
                const entryRef = firebase_config_1.default.firestore()
...
//# sourceMappingURL=add-flag-triggering-journals.js.map