...
exports.getTeacherStudents = exports.getDeliveredResources = exports.getSharedResources = exports.shareResourceWithStudents = exports.getResourceDetails = exports.getAssignedResources = exports.getTeacherInfo = exports.getTeacherAnalytics = exports.getClass = exports.getDashboard = exports.resetDiaryEntriesPin = exports.suspendOrUnsuspendStudent = exports.getStudents = exports.deleteStudent = exports.updateStudent = exports.createStudent = void 0;
...
const createStudent = async (req, res) => {
    try {
        ...
        const { role, schoolId, schoolName, districtId, districtName } = teacherClaims;
        // Validate teacher role
        if (role !== app_config_1.ROLES.TEACHER) {
            console.error('Invalid teacher role:', {
                expectedRole: app_config_1.ROLES.TEACHER,
                actualRole: role
            });
            return res.status(400).json({
                error: "Invalid teacher data",
        ...
        if (teacherClaims.classId && teacherClaims.className) {
            classId = teacherClaims.classId;
            const className = teacherClaims.className;
        ...
        if (!districtName)
            missingFields.push('districtName');
        if (!gradeName)
            missingFields.push('gradeName');
        if (!gradeId)
            missingFields.push('gradeId');
        if (!division)
        ...
        try {
            const existingUser = await firebase_config_1.default.auth().getUserByEmail(req.body.email);
            if (existingUser) {
                return res.status(400).json({
        ...
        const user = await firebase_config_1.default.auth().createUser({
            ...
            password: app_config_1.DEFAULT_PASSWORDS.STUDENT,
            emailVerified: false,
        });
        ...
        // Set custom claims
        await firebase_config_1.default.auth().setCustomUserClaims(user.uid, claims);
        ...
        await firebase_config_1.default.firestore().collection("users").doc(user.uid).set(userData);
        // Always generate password reset and email verification links
        if (req.body.email) {
            const [passwordResetLink, verifyEmailLink] = await Promise.all([
                firebase_config_1.default.auth().generatePasswordResetLink(req.body.email),
                firebase_config_1.default.auth().generateEmailVerificationLink(req.body.email)
            ]);
            // Send welcome email (prefer Titan env)
            const fromAddressForStudent = process.env.TITAN_EMAIL ||
                process.env.titanemail ||
                process.env.EMAIL ||
                "noreply@journalhood.com";
            if (nodemailer_config_1.transporter && fromAddressForStudent) {
                try {
...
const getStudents = async (req, res) => {
    try {
        ...
        const teacherData = await firebase_config_1.default.auth().getUser(uid);
        if (!teacherData.customClaims) {
            return res.status(400).json({
        ...
        const userClaims = teacherData.customClaims;
        ...
        let students = [];
        if (userRole === app_config_1.ROLES.TEACHER) {
            ...
            students = studentsSnapshot.users.filter((user) => {
                ...
                const isMatch = studentClaims.schoolId === userClaims.schoolId && // Same school
                    studentClaims.role === app_config_1.ROLES.STUDENT && // Is a student
        ...
        else if (userRole === app_config_1.ROLES.SUPER_ADMIN || userRole === app_config_1.ROLES.DISTRICT_ADMIN || userRole === app_config_1.ROLES.SCHOOL_ADMIN) {
            ...
            students = studentsSnapshot.users.filter((user) => {
                ...
                if (!studentClaims)
                    return false;
                return studentClaims.role === app_config_1.ROLES.STUDENT && (studentClaims.teacherIncharge === teacherId || (studentClaims.teacherId === teacherId && !studentClaims.teacherIncharge));
            });
        }
...
const suspendOrUnsuspendStudent = async (req, res) => {
    try {
        const { uid } = req.params;
        const user = await firebase_config_1.default.auth().getUser(uid);
        await firebase_config_1.default.auth().updateUser(uid, {
        ...
        return res.status(200).json({
            message: `Student ${!user.disabled ? "Suspended" : "Unsuspended"} successfully`,
        });
    }
    catch (error) {
        console.log(error);
...
exports.suspendOrUnsuspendStudent = suspendOrUnsuspendStudent;
// resetDiaryEntriesPin
const resetDiaryEntriesPin = async (req, res) => {
    try {
        const { uid } = req.params;
        const user = await firebase_config_1.default.auth().getUser(uid);
        await firebase_config_1.default.firestore().collection("users").doc(uid).delete();
        if (nodemailer_config_1.transporter && user.email) {
            await nodemailer_config_1.transporter.sendMail({
                from: process.env.TITAN_EMAIL || process.env.titanemail || process.env.EMAIL || 'noreply@example.com',
                to: user.email,
...
exports.resetDiaryEntriesPin = resetDiaryEntriesPin;
// getDashboard
    ...
    try {
        ...
        const students = studentsSnapshot.users.filter((user) => {
            ...
            return studentClaims.schoolId === teacherClaims.schoolId &&
                studentClaims.role === app_config_1.ROLES.STUDENT &&
        ...
        const analyticsPromises = students.map(async (student) => {
            ...
            const entries = entriesSnapshot.docs.map(doc => {
                const data = doc.data();
                const now = firebase_config_1.default.firestore.Timestamp.now();
                if (!data || !data.timestamp || !(data.timestamp instanceof firebase_config_1.default.firestore.Timestamp)) {
                    return {
                        content: '',
                        wordCount: 0,
                        timestamp: now,
                ...
                if (!(timestamp instanceof firebase_config_1.default.firestore.Timestamp)) {
                    return {
                        content: '',
                        wordCount: 0,
                        timestamp: now,
            ...
            const validEntries = entries.filter((entry) => entry.timestamp instanceof firebase_config_1.default.firestore.Timestamp);
            ...
            const prevWeekEntries = validEntries.filter(e => e.timestamp.toDate() > new Date(weekAgo.getTime() - 7 * 24 * 60 * 60 * 1000) &&
                e.timestamp.toDate() < weekAgo).length;
            const prevMonthEntries = validEntries.filter(e => e.timestamp.toDate() > new Date(monthAgo.getTime() - 30 * 24 * 60 * 60 * 1000) &&
                e.timestamp.toDate() < monthAgo).length;
...
const getTeacherAnalytics = async (req, res) => {
    try {
        ...
        console.log('🔍 Teacher Analytics Debug - Teacher Claims:', {
            ...
            division: teacherClaims.division,
            classId: teacherClaims.classId,
            className: teacherClaims.className,
...
const getAssignedResources = async (req, res) => {
    try {
        ...
        // Get teacher's custom claims
        const teacherData = await firebase_config_1.default.auth().getUser(uid);
        if (!teacherData.customClaims?.role || teacherData.customClaims.role !== app_config_1.ROLES.TEACHER) {
        ...
        const teacherClasses = await firebase_config_1.default.firestore()
            .collection('classes')
            .where('teacherId', '==', uid)
            .get();
        ...
        if (classIds.length > 0) {
            ...
            const classQueries = classChunks.map(chunk => firebase_config_1.default.firestore()
                .collection('resource_assignments')
        ...
        const resourceIds = [...new Set(allAssignments.map(doc => doc.data()?.resourceId).filter((id) => Boolean(id)))];
        ...
        for (let i = 0; i < resourceIds.length; i += 10) {
            ...
            let batchQuery = firebase_config_1.default.firestore()
                .collection('resources')
                .where(firebase_config_1.default.firestore.FieldPath.documentId(), 'in', batch);
        ...
        const resources = await Promise.all(paginatedDocs.map(async (doc) => {
            ...
            const assignment = allAssignments.find(assignmentDoc => assignmentDoc.data()?.resourceId === doc.id);
            if (assignment) {
                ...
                if (assignmentData) {
                    ...
                    if (assignmentData.assignedBy) {
                        try {
                            const assignerData = await firebase_config_1.default.auth().getUser(assignmentData.assignedBy);
...
const getResourceDetails = async (req, res) => {
    try {
        ...
        const assignmentsQuery = firebase_config_1.default.firestore()
            .collection('resource_assignments')
        ...
        if (assignments.empty) {
            ...
            if (!teacherClasses.empty) {
                const classIds = teacherClasses.docs.map(doc => doc.id);
                const classAssignmentsQuery = firebase_config_1.default.firestore()
                    .collection('resource_assignments')
                    .where('resourceId', '==', resourceId)
                    .where('targetType', '==', 'class')
                    .where('status', '==', 'active');
                const classAssignments = await classAssignmentsQuery.get();
                const hasClassAssignment = classAssignments.docs.some(doc => classIds.includes(doc.data().targetId));
...
const shareResourceWithStudents = async (req, res) => {
    try {
        const { uid } = req.user;
        ...
        if (classId) {
            ...
            const classStudents = await firebase_config_1.default.firestore()
                .collection('users')
                .where('customClaims.role', '==', app_config_1.ROLES.STUDENT)
        ...
        for (const studentId of targetStudents) {
            ...
            assignmentPromises.push(firebase_config_1.default.firestore()
                .collection('resource_assignments')
                .add(assignmentData));
        }
...
exports.shareResourceWithStudents = shareResourceWithStudents;
    ...
    try {
        ...
        const assignments = await Promise.all(paginatedDocs.map(async (doc) => {
            ...
            if (assignmentData.assignedTo) {
                try {
                    const studentData = await firebase_config_1.default.auth().getUser(assignmentData.assignedTo);
...
const getDeliveredResources = async (req, res) => {
    try {
        const { uid } = req.user;
        ...
        const students = studentsSnapshot.users.filter((user) => {
            ...
            if (!studentClaims)
                return false;
            // Check if the student belongs to this teacher
            const isMatch = studentClaims.schoolId === teacherClaims.schoolId && // Same school
                studentClaims.role === app_config_1.ROLES.STUDENT && // Is a student
                (studentClaims.teacherIncharge === uid || // Teacher is assigned directly
                    (studentClaims.teacherId === uid && !studentClaims.teacherIncharge)); // Fallback to teacherId if teacherIncharge not set
            return isMatch;
        ...
        let flagsQuery = firebase_config_1.default.firestore()
        ...
        if (studentId && typeof studentId === 'string' && studentId !== 'all') {
            flagsQuery = firebase_config_1.default.firestore()
                .collection('studentFlags')
...
exports.getDeliveredResources = getDeliveredResources;
