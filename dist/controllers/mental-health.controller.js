...
exports.getTeacherFlaggedStudents = exports.getSchoolFlaggedStudents = exports.getDistrictFlaggedStudents = exports.getMentalHealthAnalysis = exports.analyzeMentalHealth = exports.getFlaggedStudentsReport = exports.analyzeAllStudentJournals = void 0;
const firebase_config_1 = __importDefault(require("../config/firebase.config"));
const firestore_1 = require("firebase-admin/firestore");
const decrypt_journal_1 = require("../utils/decrypt-journal");
...
const getLastAnalysisState = async () => {
    try {
        const stateDoc = await firebase_config_1.default.firestore()
...
const updateAnalysisState = async (newJournalsCount, studentsCount) => {
    try {
        const stateRef = firebase_config_1.default.firestore()
            .collection('analysisState')
            .doc('mentalHealthAnalysis');
        const currentState = await getLastAnalysisState();
        await stateRef.set({
            lastAnalysisDate: firestore_1.Timestamp.now(),
...
const moveJournalsToBackup = async (journalEntries) => {
    try {
        const batch = firebase_config_1.default.firestore().batch();
        let moveCount = 0;
        for (const entry of journalEntries) {
            // Add to analyzedJournals collection
            const backupRef = firebase_config_1.default.firestore()
            ...
            const originalRef = firebase_config_1.default.firestore()
                .collection('users')
                .doc(entry.studentId)
...
const getExistingFlagCount = async (studentId, issueType) => {
    try {
        const flagSnapshot = await firebase_config_1.default.firestore()
...
const mockAnalyzeJournalEntry = (content) => {
    ...
    if (lowerContent.includes('don\'t understand') || lowerContent.includes('hard to read') ||
        ...
        (content.split(' ').length > 10 && (lowerContent.includes(' me no ') || lowerContent.includes(' i no ') ||
...
const deliverResourcesAutomatically = async (studentId, issueType, studentData) => {
    try {
        console.log(`📚 Auto-delivering resources for ${issueType} to student ${studentId}`);
        // Get resources for this issue category
        const resourcesRef = firebase_config_1.default.firestore().collection('resources');
...
const analyzeAllStudentJournals = async (req, res) => {
    try {
        ...
        const usersRef = firebase_config_1.default.firestore().collection('users');
        ...
        for (const studentDoc of studentsSnapshot.docs) {
            ...
            try {
                ...
                const journalEntriesRef = firebase_config_1.default.firestore()
                    .collection('users')
                    .doc(studentId)
                    .collection('journalEntries');
                ...
                if (lastAnalysisDate) {
                    journalQuery = journalQuery.where('timestamp', '>', firestore_1.Timestamp.fromDate(lastAnalysisDate));
                    ...
                    journalQuery = journalQuery.where('timestamp', '>=', firestore_1.Timestamp.fromDate(thirtyDaysAgo));
                ...
                const newIssueCountMap = new Map();
                const issueExcerpts = new Map();
                ...
                for (const entryDoc of journalSnapshot.docs) {
                    ...
                    if (Array.isArray(content)) {
                        ...
                        if ((0, decrypt_journal_1.isEncrypted)(content)) {
                            try {
                                const decryptedContent = (0, decrypt_journal_1.tryDecryptContent)(content, studentId);
                ...
                for (const [issueType, newCount] of newIssueCountMap.entries()) {
                    ...
                    if (totalCount >= 4) {
                        ...
                        const existingFlagsRef = firebase_config_1.default.firestore()
                            .collection('studentFlags')
                            .where('studentId', '==', studentId)
                            .where('issueType', '==', issueType);
                        ...
                        if (!existingFlagsSnapshot.empty) {
                            ...
                            flagRef = firebase_config_1.default.firestore().collection('studentFlags').doc(flagId);
                            await flagRef.set({
                                id: flagId,
                                studentId: studentId,
                            ...
                            try {
                                ...
                                if (deliveredResources && deliveredResources.length > 0) {
                                    ...
                                    await flagRef.update({
                                        ...
                                        resourcesDeliveredAt: firestore_1.Timestamp.now(),
...
exports.analyzeAllStudentJournals = analyzeAllStudentJournals;
// Get flagged students report
const getFlaggedStudentsReport = async (req, res) => {
    try {
        const { uid: adminId } = req.user;
        // Verify this is a super admin
        const adminUser = await firebase_config_1.default.auth().getUser(adminId);
        ...
        const flagsSnapshot = await firebase_config_1.default.firestore()
            .collection('studentFlags')
...
exports.getFlaggedStudentsReport = getFlaggedStudentsReport;
...
exports.analyzeMentalHealth = analyzeMentalHealth;
const getMentalHealthAnalysis = async (req, res) => {
    try {
        const { uid: teacherId } = req.user;
        ...
        const analysisRef = firebase_config_1.default.firestore().collection('mental_health_analysis');
...
exports.getMentalHealthAnalysis = getMentalHealthAnalysis;
// District Admin: Get flagged students in their district
const getDistrictFlaggedStudents = async (req, res) => {
    try {
        const { uid } = req.user;
...
exports.getDistrictFlaggedStudents = getDistrictFlaggedStudents;
// School Admin: Get flagged students in their school
const getSchoolFlaggedStudents = async (req, res) => {
    try {
        const { uid } = req.user;
        const adminData = await firebase_config_1.default.auth().getUser(uid);
...
exports.getSchoolFlaggedStudents = getSchoolFlaggedStudents;
// Teacher: Get flagged students in their class
const getTeacherFlaggedStudents = async (req, res) => {
    try {
        const { uid } = req.user;
...
exports.getTeacherFlaggedStudents = getTeacherFlaggedStudents;
