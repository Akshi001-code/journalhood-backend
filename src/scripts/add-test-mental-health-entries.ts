...
class MentalHealthTestSeeder {
    ...
    try {
      ...
      for (let i = 0; i < issueTypes.length && studentIndex < students.length; i++) {
        ...
        for (const student of studentsForThisIssue) {
          const entriesAdded = await this.addMentalHealthEntriesForStudent(student, issueType as keyof typeof this.mentalHealthTemplates);
  ...
  private async findAllStudents(): Promise<StudentData[]> {
  ...
  private async addMentalHealthEntriesForStudent(
    student: StudentData, 
    issueType: keyof typeof this.mentalHealthTemplates
  ): Promise<number> {
