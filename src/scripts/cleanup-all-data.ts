...
export class DatabaseCleaner {
   ...
   */
  async cleanup(): Promise<CleanupProgress> {
   ...
   */
  private async findSuperAdmin(): Promise<UserRecord | null> {
    try {
      ...
      try {
        ...
        if (user.customClaims?.role === ROLES.SUPER_ADMIN) {
          return user;
        }
      } catch (error) {
      ...
      do {
        const listUsersResult: ListUsersResult = await this.auth.listUsers(1000, pageToken);
   ...
   */
  private async cleanupAuthUsers(superAdminUid: string): Promise<void> {
   ...
   */
  private async cleanupFirestoreCollections(): Promise<void> {
   ...
   */
  private async cleanupDiaryEntries(): Promise<void> {
   ...
   */
  private async deleteCollection(collectionRef: any, batchSize: number = 100): Promise<void> {
